    ...
    try:
        ...
        if 'image' in request.files:
            ...
            if file and file.filename != '' and allowed_file(file.filename):
                ...
                # Générer un nom unique
                unique_filename = f"book_{new_id}_{uuid.uuid4().hex[:8]}.{file_extension}"
        ...
        if request.content_type and 'multipart/form-data' in request.content_type:
            ...
            if 'image' in request.files:
                ...
                if file and file.filename != '' and allowed_file(file.filename):
                    ...
                    old_image = books[book_index].get('image', '')
                    if old_image and '/static/uploads/images/' in old_image:
                        old_file_path = old_image.replace('/static/uploads/images/', '')
                        old_full_path = os.path.join(app.config['UPLOAD_FOLDER'], 'images', old_file_path)
                    ...
                    original_filename = secure_filename(file.filename)
                    file_extension = original_filename.rsplit('.', 1)[1].lower()
                    unique_filename = f"book_{book_id}_{uuid.uuid4().hex[:8]}.{file_extension}"
                    file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'images', unique_filename)
                    file.save(file_path)
                    image_url = f'/static/uploads/images/{unique_filename}'
