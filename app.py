...
def admin_add_book():
    ...
    try:
        ...
        if save_json_data('books.json', books):
            # Créer une notification automatique pour le nouveau livre
            try:
                notify_new_book(new_book['title'])
            except Exception as e:
...
# ==================== ROUTES DE TÉLÉCHARGEMENT GRATUIT ====================

@app.route('/api/download/book/<int:book_id>')
def download_book(book_id):
    """Téléchargement gratuit des livres PDF/Word"""
    try:
        books = load_json_data('books.json')
        ...
        # Vérifier que c'est un format téléchargeable (PDF/Word)
        if book.get('format', '').upper() not in ['PDF', 'WORD', 'DOC', 'DOCX']:
            return jsonify({
                'success': False,
                'error': 'Format de fichier non téléchargeable'
            }), 400
        
        # Simuler le téléchargement (dans un vrai cas, rediriger vers le fichier)
        return jsonify({
            'success': True,
            'download_url': f'/static/books/{book["title"]}.pdf',
            'message': 'Téléchargement gratuit disponible',
            'book': book
        })
...
@app.route('/api/download/media/<int:media_id>')
def download_media(media_id):
    """Téléchargement de médias avec vérification du type"""
    try:
        media = Media.query.get(media_id)
        ...
        # PDF et documents Word sont gratuits
        if media.file_type in ['document'] or media.original_filename.lower().endswith(('.pdf', '.doc', '.docx')):
            return jsonify({
                'success': True,
                'download_url': media.cloudinary_url,
                'message': 'Téléchargement gratuit disponible',
                'media': media.to_dict()
            })
        
        # Les vidéos nécessitent un paiement
        elif media.file_type == 'video':
            return jsonify({
                'success': False,
                'error': 'Ce contenu vidéo nécessite un achat',
                'requires_payment': True,
                'media': media.to_dict()
            }), 402  # Payment Required
        
        # Les images sont gratuites
        else:
            return jsonify({
                'success': True,
                'download_url': media.cloudinary_url,
...
class Notification(db.Model):
    ...
    notification_type = db.Column(db.String(50), nullable=False)  # 'new_book', 'new_quote', 'update', 'announcement'
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def get_notifications():
    ...
    try:
        notifications = Notification.query.filter_by(is_active=True).order_by(Notification.created_at.desc()).limit(10).all()
        return jsonify({
            'success': True,
...
@app.route('/api/admin/notifications/<int:notif_id>', methods=['PUT'])
@admin_required
...
def create_auto_notification(notification_type, title, message):
    ...
    try:
        with app.app_context():
            ...
            db.session.add(notification)
            db.session.commit()
            print(f"Notification automatique créée: {title}")
    except Exception as e:
        print(f"Erreur lors de la création de notification: {e}")

# Modifier les routes d'ajout de livres pour créer des notifications automatiques
def notify_new_book(book_title):
    """Notifier l'ajout d'un nouveau livre"""
    create_auto_notification(
        'new_book',
        'Nouveau livre disponible !',
        f'Le livre "{book_title}" est maintenant disponible en téléchargement gratuit sur notre plateforme.'
    )

def notify_new_quote():
    """Notifier l'ajout d'une nouvelle citation"""
    create_auto_notification(
        'new_quote',
        'Nouvelles citations inspirantes !',
        'De nouvelles citations ont été ajoutées à notre collection. Découvrez-les dès maintenant !'
    )

def notify_free_content():
    """Notifier que tout le contenu est gratuit"""
    create_auto_notification(
        'announcement',
        '🎉 Tout est maintenant GRATUIT !',
        'Bonne nouvelle ! Tous nos livres PDF et documents Word sont maintenant disponibles en téléchargement gratuit. Profitez-en dès maintenant !'
    )
...
if __name__ == '__main__':
    ...
    # Créer une notification de bienvenue pour le contenu gratuit
    try:
        with app.app_context():
            # Vérifier si la notification gratuite existe déjà
            existing_notif = Notification.query.filter_by(
                notification_type='announcement',
                title='🎉 Tout est maintenant GRATUIT !'
            ).first()
            
            if not existing_notif:
                notify_free_content()
    except Exception as e:
        print(f"Erreur lors de la création de la notification de bienvenue: {e}")
